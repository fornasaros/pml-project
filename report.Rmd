...
### Read the Data
```{r}
ftrain <- read.csv("./data/pml-training.csv")
ftest <- read.csv("./data/pml-testing.csv")
dim(ftrain)
dim(ftest)
```
...
## Modeling
train a **Random Forest** model with a **3-fold cross validation** to select optimal tuning parameters for the model.  
```{r}
controlRf <- trainControl(method="cv", 3,verboseIter=F)
...
fControl <- trainControl(method="cv", number=3, verboseIter=F)
